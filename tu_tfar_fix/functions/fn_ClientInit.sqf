...
if(!isNull player && {player call TFAR_fnc_hasPrototypeRadio}) then {
	[player] remoteExec ["TFAR_fnc_replaceRadios",2];
};

sleep .01; //mission start
...
if (isMultiplayer) then {
	...
	{ 
		if (isPlayer _x) then {
			[_x, false, name _x] call TFAR_fnc_sendPlayerInfo;
		}; 
...
[] spawn {
	...
	waitUntil {sleep 5;!(IsNull (findDisplay 46))};
