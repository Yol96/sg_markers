...
if (isNil "_value") then {
	private _dr = TFAR_currentUnit call TFAR_fnc_getDefaultRadioClasses;	

	if (!(TF_use_saved_lr_setting) or (isNil "TF_saved_active_lr_settings")) then {		
		if (((_dr select 0) == _radioType) or {(_dr select 3) == _radioType} or {getText(configFile >> "CfgVehicles" >> _radioType >> "tf_encryptionCode") == toLower (format ["tf_%1_radio_code",(TFAR_currentUnit call BIS_fnc_objectSide)])}) then {
	...
	if (isNil "_rc") then {
		...
		if (_hasDefaultEncryption and {((TFAR_currentUnit call BIS_fnc_objectSide) != civilian)}) then {
			if (((_dr select 0) == _radioType) or {(_dr select 3) == _radioType} or {_radio_object call TFAR_fnc_getVehicleSide == TFAR_currentUnit call BIS_fnc_objectSide}) then {
